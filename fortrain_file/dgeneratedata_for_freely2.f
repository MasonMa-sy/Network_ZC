...
cc***************************the end**********************************
c
cc-----------for testing generate data
c      CALL GENERATEDATA(NT,IT)   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!1added by mashaoyang
c
cc check to see if its time to stop
c      IF(NT.GE.NTIMES) GO TO 900
c
cc if not, loop again

      IF(MOD(NT,3).EQ.0)THEN
