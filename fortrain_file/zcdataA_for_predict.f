...
!c Declare the variables related to ZC model.
      ...
      integer i,j,k,i1,i2,irec,for_circle,predict_month,l

      real*8 tinitial,tfinal, file_num, rand_temp, start_num
...
c---end
      tinitial=48.5d0
      predict_month=12
      do l=192,457+predict_month
      tfinal=tinitial+DFLOAT(l)
      call initialbasic(toinit,h1init)
c      call bfmodel(toinit,h1init,toend,h1end)
      start_num=l-predict_month

      print *,'predict_month is ', predict_month
      
c      write(filename,'(i5.5)') int(file_num)
c      open(unit=374,file='data/data_'//filename//'.dat',
c     & form='binary',status='replace')
c      do i=6,25
c      do j=6,32
c      Write(374) TO(i,j)
c      enddo
...
c------this scrip is trying to predict.
c------the problem of oscillating is solved
c------use with assimilation
        
      call bfmodel(toinit,h1init,ssta_end,h1a_end)

      write(filename,'(i5.5)') int(l-192)
